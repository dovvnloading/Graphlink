...
- `backend/agents.py` (now the largest file in the repo, ~156KB)
  - `AgentDispatcher` (one instance per session, never a module-level singleton) - owns in-flight request tracking/cancellation across its 13 dispatch surfaces, `bootstrap_provider_state()` (process-global `api_provider` state, set up once per process from the shared `SettingsManager`), and `register_agents()`. As of ADR-002 stage 2.4, 10 surfaces (chat/conversation, image, artifact, web research, gitlink run, gitlink apply, chart, note, branch comparison, branch synthesis) claim into one shared `self._runs` (a `backend/run_lifecycle.py` `RunRegistry`) instead of their own private dict - chat and artifact are the first two `cancel_event`-bearing kinds to coexist there (exercising `RunRegistry.cancel()`'s `kind=` filter), web research is the first to use `RunHandle.on_cancel` (its cancellation primitive is a `CancellationToken`, not a `threading.Event`), and gitlink run/apply are the first two kinds whose real busy guard is `node.pending_request_id` (a per-SceneNode field) rather than the registry's own `is_busy()` - the registry there is pure task/cancel_event bookkeeping. Only pycoder + code sandbox remain, deferred to the final slice of stage 2.4 (they need the `approval_future` field added in stage 2.4b).
