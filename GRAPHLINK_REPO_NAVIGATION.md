...
- `backend/agents.py` (now the largest file in the repo, ~156KB)
  - `AgentDispatcher` (one instance per session, never a module-level singleton) - owns in-flight request tracking/cancellation across its 13 dispatch surfaces, `bootstrap_provider_state()` (process-global `api_provider` state, set up once per process from the shared `SettingsManager`), and `register_agents()`. As of ADR-002 stage 2.4, 8 surfaces (chat/conversation, image, artifact, web research, chart, note, branch comparison, branch synthesis) claim into one shared `self._runs` (a `backend/run_lifecycle.py` `RunRegistry`) instead of their own private dict - chat and artifact are the first two `cancel_event`-bearing kinds to coexist there (exercising `RunRegistry.cancel()`'s `kind=` filter), and web research is the first to use `RunHandle.on_cancel` (its cancellation primitive is a `CancellationToken`, not a `threading.Event`). The remaining 4 fire-and-forget surfaces (gitlink x2, pycoder, code sandbox) still keep independent dicts, deferred to the rest of stage 2.4.
