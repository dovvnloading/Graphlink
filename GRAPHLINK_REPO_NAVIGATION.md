...
Last refreshed: 2026-08-25 (file/test counts, CI job list, and the attachments/domain-file claims below re-verified against the current repo; the rest of the document has not been re-audited past 2026-07-27 and may carry further drift not yet caught)
...
- Real top-level layout:
  - `backend/` - real Python package (`__init__.py` present). ALL application/domain logic: the FastAPI app, the WebSocket event bus, and every node/canvas/agent/settings/persistence model. `backend/tests/` holds its pytest suite (98 files directly under it, plus 2 more under `backend/tests/perf/` - 100 total, as of 2026-08-25; the "25 files" this line used to say was itself stale by this point).
  ...
  - `tests/` - not a package (no `__init__.py`). As of 2026-08-25 holds 8 test files (`test_dependency_lock_currency.py`, `test_doc_currency.py`, `test_domain_purity.py`, `test_graphlink_desktop.py`, `test_no_qt_anywhere.py`, `test_node_state_migration.py`, `test_register_function_length.py`, `test_undo_classification_gate.py`) plus one non-test helper module (`undo_classification.py`) - `test_no_qt_anywhere.py` (the permanent Qt-removal gate) is the original and best-known one, but no longer the only file here.
  - `mutation_tests/` - a separate top-level pytest-collected suite (not mentioned by any of this doc's older passes), currently 20 tests; also collected by a bare `python -m pytest -q` from the repo root alongside `backend/tests/`, `contracts/tests/`, and `tests/` above.
...
- CI (`.github/workflows/ci.yml`): as of 2026-08-25, four jobs, only one on `windows-latest` (this line used to say "two jobs, both windows-latest" - stale on both counts): **`python-tests`** ("Python checks (Windows, DPAPI)", `windows-latest` - renamed away from its old Qt-removal-era "Python tests (offscreen)" name at ADR-015 stage 15.3, since nothing here is offscreen-mode-related anymore) runs `pip install -r requirements.txt`, `compileall`, `ruff`, `mypy`, then `python -m pytest -q --cov ... --ignore=backend/tests/perf` plus a separate uninstrumented `backend/tests/perf` run and a `pip-audit` scan; a bare `python -m pytest -q` from the repo root (no ignores) collects `backend/tests/` (incl. `backend/tests/perf/`), `contracts/tests/`, `tests/`, and `mutation_tests/` - 3089 tests total (not 961). **`build-check`** ("Build check...", `ubuntu-latest` - moved off Windows 2026-08-07 for the 2x billing-rate minutes economy, since nothing it does is Windows-bound) builds the wheel, `twine check`s it, and installs+imports it into a clean venv. **`frontend-checks`** ("Frontend checks (npm run check)", `ubuntu-latest`, same minutes-economy reasoning) runs `npm ci` then `npm run check` inside `web_ui/` (confirmed by direct run: 1991 vitest tests across 83 files, not 779/41 - all passing). **`e2e-playwright`** ("E2E (Playwright boot-smoke)", `ubuntu-latest`) is net-new since this line was last checked. `windows-latest` is pinned **solely** for `python-tests`' real Windows DPAPI tests (`backend/tests/test_backend_secrets_at_rest.py`) - there is no remaining Qt/offscreen-mode reason, and the other three jobs are deliberately NOT on Windows.
...
### 10. Message-composer attachments are real and live (R8a); a separate, still-unreached path is stand-alone document-NODE creation

As of 2026-08-25 this is only half true, and was fully true when last checked - re-verify claims like this rather than trusting them: **composer attachments shipped (R8a)** and are reachable from the real UI today. `backend/attachments.py` (ported faithfully from the deleted legacy `graphlink_file_handler.py`/`_stage_attachment_file`) classifies and extracts staged files - image/audio go in as multimodal content-parts, documents (~44 text/code extensions, `.pdf` via `pypdf`, `.docx` via `python-docx`, or a byte-sniff fallback) are extracted to text. `backend/composer.py`'s payload sets `"attachments": True` (not `False`) in its capabilities, and `web_ui/src/app/chrome/Composer.tsx` renders a real attach button wired to `composerStore.ts`'s `attachFile()`, which opens a native OS file dialog (`backend/native_dialogs.py`) server-side. `reportlab` is the one dependency of the three originally named here that is STILL unused anywhere in the codebase (confirmed: zero references under `backend/`) - a genuine legacy-holdover dependency; `pypdf`/`python-docx` are not.

Separately, and still accurate: `backend/domain/` has a full document-NODE model (pdf/docx/audio metadata, `backend/domain/model.py` + `graph.py`) and a wired `addDocumentNode` WS intent, but `web_ui/src/app/canvas/sceneStore.ts`'s `addDocumentNode()` (and its 4 siblings - `addThinkingNode`/`addHtmlNode`/`addImageNode`/`addConversationNode`) is only ever called from its own test file - no real UI component invokes any of them, and this is a genuinely separate capability from message-composer attachments above (adding a whole document/thinking/html/image/conversation NODE to the canvas directly, vs. staging a file onto a chat message). Investigated during a 2026-08-25 tech-debt sweep and deliberately left as-is, not deleted: each method has real, dedicated unit-test coverage (a maintained contract, not orphaned code) and its own doc comment consistently frames the gap as "no live UI trigger yet," matching a backend-first, trigger-deferred delivery pattern this codebase uses elsewhere - removing them would discard tested capability for no real benefit.
...
- `backend/domain/` + `backend/canvas.py` (split at ADR-002 stage 2.2; before it, canvas.py was one ~5,100-line file owning both halves)
  - `backend/domain/`: `SceneNode`/`SceneEdge` (`model.py`), `SceneDocument` (`graph.py`, composed as `class SceneDocument(BranchOps, GroupOps, LayoutOps, CommandOps)` - two more mixins joined since this line was last checked) from `branches.py`/`groups.py`/`layout.py`/`commands.py` - every node kind's creation/deletion/reparenting, the unified edge model, frame/container/note/chart creation, branch-root resolution, collapse/expand, view-state, size-aware node placement/organize (`layout.py`, ADR's rebuilt placement engine - `node_footprint()`/`find_free_position()`), and the undo/redo command layer (`commands.py`, ADR-010 stage 10.1 - scoped before/after diffs, not hand-authored inverses). Purity gated by `tests/test_domain_purity.py`.
...
- `domain/` - the pure scene domain, split out of canvas.py at ADR-002 stage 2.2 and permanently gated by `tests/test_domain_purity.py` (AST check: no fastapi/events/notifications/agents/token_counter imports, no bus-shaped calls): `model.py` (`SceneNode`, `SceneEdge`, errors, layout/appearance constants), `content_codec.py` (the shared `_content_codec` namespace - ONE instance), `graph.py` (`SceneDocument`, composed as `class SceneDocument(BranchOps, GroupOps, LayoutOps, CommandOps)` with core node/edge/chart/view-state methods), `branches.py` (`BranchOps`: branch-tree semantics, `send_message`/`chat_branch_history`/`delete_chat_node`/branch status), `groups.py` (`GroupOps`: frame/container geometry), `layout.py` (`LayoutOps`: size-aware node placement/organize - `node_footprint()`, `find_free_position()`), `commands.py` (`CommandOps`: the undo/redo command layer, ADR-010 stage 10.1 - scoped before/after diffs), `node_states.py` (typed per-kind `SceneNode.state` dataclasses, ADR-002 stage 2.5, migrated kind-by-kind off SceneNode's own ~95-field catch-all). Patch seams: names bound in `domain/*` resolve in THOSE modules' namespaces - patch `backend.domain.graph.<name>`, never `backend.canvas.<name>`, for document behavior.
...
- `tests/` - the backend pytest suite (98 files directly here plus 2 more under `tests/perf/` - 100 total, as of 2026-08-25).
...
- Verify with the CI command, not a narrowed one: `python -m pytest -q` from the repo root (3089 tests as of 2026-08-25, across `backend/tests/`, `contracts/tests/`, `tests/`, and `mutation_tests/` - re-check this count, it has drifted before), plus `npm run check` from inside `web_ui/` (schema-drift check, typecheck, lint, 1991 vitest tests across 83 files as of 2026-08-25, build). A subset run can look green while missing real regressions in a directory it never collected.
