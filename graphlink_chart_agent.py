"""Qt-free chart-data-agent core (Qt-removal plan R6.2 prerequisite).

`ChartDataAgent` moved out of graphlink_agents_tools.py: that module's
unconditional `from PySide6.QtCore import QThread, Signal` (needed only by
its `ChartWorkerThread`/`ImageGenerationWorkerThread`/`ModelPullWorkerThread`
classes) meant importing anything from it - including this Qt-free class -
pulled PySide6 into the process. That made it unimportable from backend/
despite containing zero Qt code itself, the exact same problem R5.2 fixed for
Artifact by splitting graphlink_artifact_agent.py out of
graphlink_agents_artifact.py (see that module's own docstring) - this is the
same split, applied to the chart agent.

graphlink_agents_tools.py re-exports this class unchanged for its own
ChartWorkerThread (which constructs one internally) and the legacy Qt call
site (graphlink_window_actions.py, which imports ChartWorkerThread) - see
its own import line.
...
# task/provider/model config. graphlink_config itself chains to
# PySide6.QtGui/QtWidgets (theming) - importing it here would silently
# re-taint this module with Qt despite containing no Qt code, defeating the
# entire point of this extraction (see the module's own docstring above).
...
"""
    ...
    def _build_chart_system_prompt(self, chart_type):
        return (
            "You are Graphlink's chart payload extractor.\n"
    ...
    def _build_chart_repair_prompt(self, chart_type):
        return (
            "You repair malformed chart JSON into a strict Graphlink chart payload.\n"
