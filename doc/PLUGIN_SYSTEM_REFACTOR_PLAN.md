...
**Date:** 2026-07-07
**Status:** Phases 0-2 complete, Phase 3 in progress (3a done, 3b next). Fixed so far: Workflow allowlist drift for Code Review Agent (§1.4/§4.7), Gitlink's write-approval state machine + path-safety tests (§2.2/§4.4), Code Sandbox's human-approval gate (§2.1/§4.3), the additive `PluginSpec`/`PLUGIN_REGISTRY` table (§3.1/§4.8), the `create_node()` generic factory with Artifact migrated onto it (§3.2/§4.1/§4.8), the `seed_prompt()` protocol replacing the isinstance dispatch chain for all 11 seedable plugins (§3.4/§4.7), Graph Diff's node-label map migrated onto the registry, fixing a real missing-`CodeReviewNode` display bug in the process (§4.5), and the shared `GitHubRestClient` replacing Code Review's and Gitlink's independently hand-rolled GitHub API logic (§1.6/§4.2/§4.4). 73 tests now cover this work (`graphite_app/tests/`). Still open: Code Sandbox OS-level hardening (deferred pending a scope decision), per-node scene bookkeeping (§3.3 — deliberately deferred, see §4.8's Phase 2 note for why), the shared LLM-JSON-agent base for Code Review/Quality Gate (§3.5, Phase 3b), and migrating the remaining 12 plugins' factories/metadata onto the registry one at a time (§5 Phase 4-5).
...
2. `main_window.artifact_thread` is a single shared attribute ([graphite_window_actions.py:1311](C:/Users/Admin/source/repos/graphite_app/graphite_app/graphite_window_actions.py#L1311)), not per-node — a second concurrent Artifact node overwrites the first's thread reference with nothing tracking or cancelling the original. Still open.
3. The "stop" icon shown by `set_running_state(True)` ([~line 471-478](C:/Users/Admin/source/repos/graphite_app/graphite_app/graphite_plugins/graphite_plugin_artifact.py#L471)) has no click handler wired to actually stop anything — purely cosmetic, implies a cancel capability that doesn't exist. Still open.
...
**✅ Phase 3a done (GitHub client slice):** the GitHub-client half of the "low-risk first slice" above is complete — see §1.6. The scoring-engine half (extracting `CodeReviewAnalyzer` and factoring it against `QualityGateAnalyzer`) is Phase 3b, tracked separately since scoring/rubric logic carries more behavioral risk than networking boilerplate.
...
**Refactor Plan:** ~~Extract `gitlink_github_client.py` (shared with Code Review per §3.5).~~ **✅ Done (Phase 3a)** — see §1.6; `GitlinkNode` now delegates to `graphite_plugins/common/github_client.py`'s `GitHubRestClient`, same as Code Review. Extract `gitlink_context_builder.py` (XML context assembly, pure functions, unit-testable independent of the node) — still open. Formalize the write gate as `gitlink_write_gate.py`: an explicit `DRAFT → PREVIEWED → APPROVED → APPLIED` state machine that fingerprints the exact file list shown to the user and refuses to apply unless the fingerprint still matches at write time — **✅ done in Phase 0**, see §2.2.
...
4. **Phase 3 (shared engines) — in progress:** ✅ extract `common/github_client.py` (§3.5) — done, Code Review and Gitlink both migrated onto it (§1.6). ⬜ extract `common/llm_json_agent.py` and migrate Code Review's and Quality Gate's analyzers onto it — not yet done.
5. **Phase 4 (finish the registry migration, not yet started):** migrate the remaining plugins (Code Sandbox, Workflow, Quality Gate, Code Review, Gitlink) onto `create_node()`/the registry one at a time; each migration should delete that plugin's entries from the hardcoded isinstance chains in `graphite_scene.py`/`graphite_window.py`/`graphite_window_actions.py`/session files as it goes, rather than leaving both paths live indefinitely. Expect to hit the same portal↔plugin circular-import constraint discovered in §4.5 for any plugin that wants to consume the registry from its own module — either keep using a deferred import per plugin, or revisit whether `PLUGIN_REGISTRY` should move to a dependency-free module with self-registration (each plugin file registers itself, rather than `graphite_plugin_portal.py` importing every plugin to build the table) if the deferred-import workaround starts feeling repetitive across enough plugins.
6. **Phase 5 (cleanup, not yet started):** once all 9 plugins are on the registry, delete the now-empty hardcoded factory methods in `graphite_plugin_portal.py`, resolve the dual import paths (§1.5), move `graphite_plugin_context_menu.py` into the package (§4.9), and implement per-node scene bookkeeping (§3.3, deferred from Phase 2).
