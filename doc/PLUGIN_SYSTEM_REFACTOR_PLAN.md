...
**Date:** 2026-07-07
**Status:** Phases 0-2 complete. Fixed so far: Workflow allowlist drift for Code Review Agent (§1.4/§4.7), Gitlink's write-approval state machine + path-safety tests (§2.2/§4.4), Code Sandbox's human-approval gate (§2.1/§4.3), the additive `PluginSpec`/`PLUGIN_REGISTRY` table (§3.1/§4.8), the `create_node()` generic factory with Artifact migrated onto it (§3.2/§4.1/§4.8), the `seed_prompt()` protocol replacing the isinstance dispatch chain for all 11 seedable plugins (§3.4/§4.7), and Graph Diff's node-label map migrated onto the registry, fixing a real missing-`CodeReviewNode` display bug in the process (§4.5). 54 tests now cover this work (`graphite_app/tests/`). Still open: Code Sandbox OS-level hardening (deferred pending a scope decision), per-node scene bookkeeping (§3.3 — deliberately deferred, see §4.8's Phase 2 note for why), and migrating the remaining 12 plugins' factories/metadata onto the registry one at a time (§5 Phase 3-5).
...
4. **Phase 3 (shared engines, not yet started):** extract `common/github_client.py` and `common/llm_json_agent.py` (§3.5); migrate Code Review, Gitlink, and Quality Gate onto them independently of the registry work.
