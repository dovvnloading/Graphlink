...
SPARSE ROWS: a payload dataclass that sets `WIRE_OMITS_DEFAULTS = True` (a
ClassVar, so never a field) is sent with every field that sits at its declared
default left out, and the receiver restores those from the same declaration -
the scene topic's SceneNodeRow, whose sender is backend/domain/node_wire.py and
whose client-side restore is generated by contracts/codegen.py. For such a row
a defaulted field is not required on the wire; its schema property carries the
restored value as `default`. Every other dataclass is unaffected.
"""
...
def omits_defaults(dataclass_type: type) -> bool:
    """True for a sparse row - see this module's docstring."""
    return bool(getattr(dataclass_type, "WIRE_OMITS_DEFAULTS", False))


NO_DEFAULT = object()


def wire_default(field: dataclasses.Field[Any]) -> Any:
    """The JSON value a receiver restores for `field` when a sparse row omits
    it, or NO_DEFAULT when the field declares none (always sent). A nested
    dataclass default becomes its dict with every None field left out - the
    shape the sender uses for an unset nested row (SceneNodeRow.chartData is
    `{}` for every non-chart node). Container defaults must be EMPTY: a
    receiver restores them as fresh empty values, never shared instances."""
    ...
    if dataclasses.is_dataclass(value) and not isinstance(value, type):
        value = {k: v for k, v in dataclasses.asdict(value).items() if v is not None}
    if isinstance(value, (list, dict)) and value:
        raise SchemaGenerationError(
            f"{field.name}: a sparse row's container default must be empty, got {value!r}"
        )
    return value
...
def validate_payload(payload: Any, dataclass_type: type, *, _path: str = "$") -> list[str]:
    ...
    for field in dataclasses.fields(dataclass_type):
        ...
        if field.name not in payload:
            if not is_optional and not (sparse and wire_default(field) is not NO_DEFAULT):
