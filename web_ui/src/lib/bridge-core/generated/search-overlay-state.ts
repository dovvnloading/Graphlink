/* GENERATED - do not hand-edit. Source of truth: graphlink_app/graphlink_search_overlay_payload.py::SearchOverlayStatePayload.
...
function checkSearchOverlayState(value: unknown, path: string, errors: string[]): void {
  ...
  {
    const fieldValue = value["currentIndex"];
    if (fieldValue === undefined || fieldValue === null) errors.push(`${path}.currentIndex: missing required field`);
    else { if (typeof fieldValue !== "number") errors.push(`${path}.currentIndex` + ": expected number"); }
  }
  {
    const fieldValue = value["totalMatches"];
    if (fieldValue === undefined || fieldValue === null) errors.push(`${path}.totalMatches: missing required field`);
    else { if (typeof fieldValue !== "number") errors.push(`${path}.totalMatches` + ": expected number"); }
  }
  {
...
export function validateSearchOverlayState(value: unknown): ValidationResult<SearchOverlayState> {
  const errors: string[] = [];
  checkSearchOverlayState(value, "$", errors);
  return errors.length === 0
    ? { ok: true, value: value as SearchOverlayState }
    : { ok: false, errors };
}
