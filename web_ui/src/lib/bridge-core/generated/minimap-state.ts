/* GENERATED - do not hand-edit. Source of truth: graphlink_app/graphlink_minimap_payload.py::MinimapStatePayload.
...
export interface MinimapNodeEntry {
  id: string;
  isUser: boolean;
  preview: string;
}
...
function checkMinimapNodeEntry(value: unknown, path: string, errors: string[]): void {
...
function checkMinimapState(value: unknown, path: string, errors: string[]): void {
  if (!isRecord(value)) { errors.push(`${path}: expected object`); return; }
  {
  ...
  }
  {
    ...
    else (fieldValue as unknown[]).forEach((item, i) => { checkMinimapNodeEntry(item, `${path}.nodes` + `[${i}]`, errors); }); }
  }
  {
...
export function validateMinimapState(value: unknown): ValidationResult<MinimapState> {
  const errors: string[] = [];
  checkMinimapState(value, "$", errors);
  return errors.length === 0
    ? { ok: true, value: value as MinimapState }
    : { ok: false, errors };
}
