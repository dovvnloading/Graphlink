 ...
 * starting; truthy -> null/undefined is a run ending. Only code_sandbox/
 * gitlink/harness get a kind-specific error check on the ending edge: those
 * are the sandboxed/external-side-effect kinds where "did it actually work"
 * is the thing a non-visual user most needs confirmed (a failed chat reply
 * already gets its own screen-reader-visible retry affordance in the
 * transcript; these don't have an equivalent without this). `builderStatus`
 * (the Plan node) is a second, independent field this also watches, since
 * Plan runs don't use pendingRequestId at all.
 */
