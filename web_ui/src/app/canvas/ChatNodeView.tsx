...
export interface ChatNodeData extends Record<string, unknown> {
  ...
  // ADR-018 stage 18.3: the opposite direction from provider/model above -
  // an explicit PIN deciding what the NEXT reply (or, when this node is a
  // branch root, the branch) resolves to, not provenance of a past one.
  // "" for both means "no pin here" (the overwhelming majority of nodes).
  overrideProvider: string;
  overrideModelId: string;
  onPinToCurrentModel: () => void;
...
function ChatNodeMenu({
  ...
  return (
    <NodeMenu position={position} onClose={onClose} className="chat-node-menu">
      ...
      {/* ADR-018 stage 18.3: pins this node (and, when it is a branch root,
          every descendant that doesn't pin its own - see backend/domain/
          branches.py's resolve_model_for_node) to whatever model is
          CURRENTLY active in the Composer. A full "browse every installed
          model" picker needs a live catalog fetch this stage doesn't wire
          to the frontend yet - see doc/adr/ADR-018-model-routing.md's own
          status note; "pin to current" is the buildable, honestly-scoped
          slice of the feature today. */}
      <button
        type="button"
        role="menuitem"
        onClick={() => {
      ...
      {overrideModelId && (
        ...
        >
          Clear Model Pin ({overrideModelId})
        </button>
      )}
...
export function chatNodePropsAreEqual(prev: NodeProps<ChatFlowNode>, next: NodeProps<ChatFlowNode>): boolean {
  ...
  return (
    ...
    a.model === b.model &&
    a.overrideProvider === b.overrideProvider &&
    a.overrideModelId === b.overrideModelId &&
    ...
    a.onPinToCurrentModel === b.onPinToCurrentModel &&
    a.onClearModelOverride === b.onClearModelOverride &&
    ...
    <NodeShell
      ...
      header={
        ...
        <span className="chat-node-role-group">
          ...
          {data.overrideModelId && (
            // ADR-018 stage 18.3: a distinct badge from chat-node-model-
            // badge above - that one is OUTPUT provenance (what a completed
            // reply was generated by), this is an INPUT pin (what the NEXT
            // reply will be generated by); both can legitimately show at
            // once on a node whose last reply predates its current pin.
            <span
              className="chat-node-model-override-badge"
              title={`Pinned to ${data.overrideProvider} - ${data.overrideModelId}`}
            >
              📌 {data.overrideModelId}
            </span>
          )}
