...
try:
    from packaging.version import InvalidVersion, Version as _PackagedVersion
except Exception:  # pragma: no cover - optional dependency fallback
    InvalidVersion = Exception
    _PackagedVersion = None

_VERSION_PATTERN = re.compile(r"^\s*v?(\d+(?:\.\d+)*)\s*$", re.IGNORECASE)
_NUMERIC_FALLBACK_PATTERN = re.compile(r"\d+")
...
def compare_versions(current_version: str, remote_version: str):
    ...
    if _PackagedVersion is not None:
        try:
            current_parsed = _PackagedVersion(normalized_current)
            remote_parsed = _PackagedVersion(normalized_remote)
        except InvalidVersion:
            current_parsed = None
            remote_parsed = None

        if current_parsed is not None and remote_parsed is not None:
    ...
    if current_parts is None or remote_parts is None:
        current_parts = tuple(int(match) for match in _NUMERIC_FALLBACK_PATTERN.findall(normalized_current))
        remote_parts = tuple(int(match) for match in _NUMERIC_FALLBACK_PATTERN.findall(normalized_remote))

        if normalized_current == normalized_remote:
            return 0
        if not current_parts and not remote_parts:
            return 0
