...
graphite_plugin_code_review.py and graphite_plugin_gitlink.py used to each hand-roll
...
class TestRequest:
    def test_successful_request_returns_json(self):
        ...
        with patch("graphite_plugins.common.github_client.requests.get", return_value=_fake_response(200, json_data={"ok": True})) as mock_get:
    ...
    def test_expect_json_false_returns_raw_content(self):
        client = GitHubRestClient(FakeSettingsManager())
        with patch("graphite_plugins.common.github_client.requests.get", return_value=_fake_response(200, content=b"raw-bytes")):
            result = client.request("https://api.github.com/x", expect_json=False)
        assert result == b"raw-bytes"

    def test_404_raises_specific_message(self):
        client = GitHubRestClient(FakeSettingsManager())
        with patch("graphite_plugins.common.github_client.requests.get", return_value=_fake_response(404, json_data={"message": "Not Found"})):
    ...
    def test_401_raises_specific_message(self):
        client = GitHubRestClient(FakeSettingsManager())
        with patch("graphite_plugins.common.github_client.requests.get", return_value=_fake_response(401, json_data={"message": "Bad credentials"})):
    ...
    def test_403_with_rate_limit_message_raises_rate_limit_error(self):
        client = GitHubRestClient(FakeSettingsManager())
        with patch("graphite_plugins.common.github_client.requests.get", return_value=_fake_response(403, json_data={"message": "API rate limit exceeded"})):
            with pytest.raises(RuntimeError, match="rate limit reached"):
                client.request("https://api.github.com/x")

    def test_403_without_rate_limit_message_raises_raw_message(self):
        client = GitHubRestClient(FakeSettingsManager())
        with patch("graphite_plugins.common.github_client.requests.get", return_value=_fake_response(403, json_data={"message": "Forbidden for another reason"})):
            with pytest.raises(RuntimeError, match="Forbidden for another reason"):
                client.request("https://api.github.com/x")

    def test_non_json_error_body_falls_back_to_text(self):
        client = GitHubRestClient(FakeSettingsManager())
        with patch("graphite_plugins.common.github_client.requests.get", return_value=_fake_response(500, json_data=None, text="Internal Server Error")):
            with pytest.raises(RuntimeError, match="Internal Server Error"):
                client.request("https://api.github.com/x")

    def test_passes_params_and_timeout_through(self):
        client = GitHubRestClient(FakeSettingsManager())
        with patch("graphite_plugins.common.github_client.requests.get", return_value=_fake_response(200, json_data={})) as mock_get:
