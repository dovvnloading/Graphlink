...
class _FakeWindow:
    ...
    _iter_shutdown_threads = graphite_window.ChatWindow._iter_shutdown_threads
    _request_thread_shutdown = graphite_window.ChatWindow._request_thread_shutdown
    _shutdown_background_threads = graphite_window.ChatWindow._shutdown_background_threads
...
class TestIterShutdownThreadsHasNoGhostAttributes:
    ...
    def test_removed_plugin_thread_names_are_not_in_source(self):
        source = Path(graphite_window.__file__).read_text(encoding="utf-8")
        for dead_attr in (
            "reasoning_thread",
            "workflow_thread",
            "graph_diff_thread",
            ...
            assert dead_attr not in source, (
                f"'{dead_attr}' reappeared in graphite_window.py - these plugins moved "
