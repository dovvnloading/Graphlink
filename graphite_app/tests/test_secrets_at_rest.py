"""Tests for secrets-at-rest encryption (graphite_secrets + SettingsManager wiring).
...
class TestProtectUnprotectPrimitives:
    def test_roundtrip(self):
        protected = graphite_secrets.protect("sk-super-secret")
        assert graphite_secrets.unprotect(protected) == "sk-super-secret"
    ...
    def test_protect_is_idempotent(self):
        once = graphite_secrets.protect("sk-super-secret")
        assert graphite_secrets.protect(once) == once

    def test_legacy_plaintext_passes_through_unprotect_unchanged(self):
        assert graphite_secrets.unprotect("sk-legacy-plaintext") == "sk-legacy-plaintext"

    def test_empty_values_stay_empty(self):
        assert graphite_secrets.protect("") == ""
        assert graphite_secrets.unprotect("") == ""
    ...
    def test_undecryptable_blob_returns_empty_not_garbage(self):
        ...
        assert graphite_secrets.unprotect("dpapi:AAAA") == ""
        assert graphite_secrets.unprotect("dpapi:!!!not-base64!!!") == ""

    def test_unicode_secrets_roundtrip(self):
        secret = "pässwörd-秘密-🔑"
        assert graphite_secrets.unprotect(graphite_secrets.protect(secret)) == secret
    ...
    def test_plaintext_secret_that_starts_with_the_prefix_is_still_encrypted(self):
        ...
        protected = graphite_secrets.protect(secret)

        assert protected != secret  # it was actually encrypted, not passed through
        assert graphite_secrets.unprotect(protected) == secret  # ...and round-trips
    ...
    def test_prefixed_plaintext_with_base64_valid_suffix_is_still_encrypted(self):
        ...
        assert graphite_secrets.unprotect(protected) == secret
...
class TestGracefulDegradationWithoutDpapi:
    def test_everything_behaves_like_before_when_dpapi_is_unavailable(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(graphite_secrets, "_dpapi_call", lambda data, encrypt: None)
