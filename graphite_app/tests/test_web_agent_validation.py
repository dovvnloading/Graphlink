...
from graphite_agents_web import WebSearchAgent
...
class TestValidateContent:
    def test_explicit_unsafe_is_rejected(self):
        ...
        with patch("graphite_agents_web.api_provider.chat", return_value=_chat_response("UNSAFE")):
            assert agent.validate_content("query", "content") is False

    def test_explicit_safe_is_accepted(self):
        agent = WebSearchAgent.__new__(WebSearchAgent)
        agent.validation_prompt = "system prompt"
        with patch("graphite_agents_web.api_provider.chat", return_value=_chat_response("SAFE")):
            assert agent.validate_content("query", "content") is True

    def test_unsafe_with_surrounding_text_is_rejected(self):
        agent = WebSearchAgent.__new__(WebSearchAgent)
        agent.validation_prompt = "system prompt"
        with patch("graphite_agents_web.api_provider.chat", return_value=_chat_response("Verdict: UNSAFE.")):
            assert agent.validate_content("query", "content") is False

    def test_safe_with_surrounding_text_is_accepted(self):
        agent = WebSearchAgent.__new__(WebSearchAgent)
        agent.validation_prompt = "system prompt"
        with patch("graphite_agents_web.api_provider.chat", return_value=_chat_response("This is SAFE.")):
            assert agent.validate_content("query", "content") is True
