"""Integration tests for ComposerContextHost - Phase 5 increment 3.

See test_composer_picker_web_host.py's module docstring for the shared
rationale (embedded host, dismiss_on_outside_focus for outside-click-close
parity with the deleted native ComposerContextPopup).
"""
...
from graphlink_composer_context_bridge import COMPOSER_CONTEXT_MAX_HEIGHT
from graphlink_composer_context_web import COMPOSER_CONTEXT_WIDTH, ComposerContextHost
...
def _make_host(parent=None) -> ComposerContextHost:
    return ComposerContextHost(_FakeComposerBridge(), parent=parent)
...
def test_construction_sizes_to_fixed_width():
    ...
    assert host.width() == COMPOSER_CONTEXT_WIDTH == 440
...
def test_height_negotiation_bounds_are_wired_to_the_bridge():
    ...
    assert host.height() == COMPOSER_CONTEXT_MAX_HEIGHT
