"""Integration tests for GridControlHost - Phase 6 increment 4.

Like PluginPickerHost/ComposerPickerHost, this is an EMBEDDED host (no
Window flag) - matching the legacy GridControl QWidget's own plain,
no-outside-click-dismiss behavior (only ever shown/hidden via the toolbar's
Controls-toggle intent).
"""
...
from graphlink_grid_control_bridge import GRID_CONTROL_MAX_HEIGHT
from graphlink_grid_control_web import GRID_CONTROL_WIDTH, GridControlHost
...
def _make_host(parent=None) -> GridControlHost:
    return GridControlHost(_FakeChatView(), parent=parent)
...
def test_construction_sizes_to_fixed_width():
    ...
    assert host.width() == GRID_CONTROL_WIDTH == 220
...
def test_height_negotiation_bounds_are_wired_to_the_bridge():
    ...
    assert host.height() == GRID_CONTROL_MAX_HEIGHT
