"""The drag-speed wire contract: staleness of the generated artifacts, and
...
from graphlink_drag_speed_bridge import DragSpeedBridge
from graphlink_drag_speed_payload import DragSpeedStatePayload
...
_SCHEMA_FILE = _GENERATED / "drag-speed-state.schema.json"
_TS_FILE = _GENERATED / "drag-speed-state.ts"

_TS_SOURCE_LABEL = "graphlink_app/graphlink_drag_speed_payload.py::DragSpeedStatePayload"

_REGENERATE_HINT = (
    "Regenerate with graphlink_island_codegen.py's schema_json_for()/"
    "typescript_for() against DragSpeedStatePayload and commit the result."
)
...
class TestTheContractDescribesTheRealPayload:
    def test_a_real_bridge_snapshot_validates(self):
        errors = validate_payload(_snapshot(), DragSpeedStatePayload)
...
class TestValidatorActuallyRejects:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (lambda p: p.pop("percentPresets"), "missing required field"),
            (lambda p: p.__setitem__("percentMin", "not an int"), "expected integer"),
            (lambda p: p["percentPresets"].__setitem__(0, "not an int"), "expected integer"),
    ...
    def test_mutation_is_caught(self, mutate, expected_fragment):
        ...
        errors = validate_payload(payload, DragSpeedStatePayload)
...
class TestGeneratedArtifactsAreNotStale:
    ...
    def test_schema_matches_regenerating_it_now(self):
        fresh = schema_json_for(DragSpeedStatePayload, title="DragSpeedState")
    ...
    def test_typescript_matches_regenerating_it_now(self):
        fresh = typescript_for(DragSpeedStatePayload, source=_TS_SOURCE_LABEL)
