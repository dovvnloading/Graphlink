"""The document-viewer wire contract: staleness of the generated artifacts,
and that the contract actually describes the REAL payload.
...
from graphlink_document_viewer_payload import DocumentViewerStatePayload
...
_SCHEMA_FILE = _GENERATED / "document-viewer-state.schema.json"
_TS_FILE = _GENERATED / "document-viewer-state.ts"

_TS_SOURCE_LABEL = "graphlink_app/graphlink_document_viewer_payload.py::DocumentViewerStatePayload"

_REGENERATE_HINT = (
    "Regenerate with graphlink_island_codegen.py's schema_json_for()/"
    "typescript_for() against DocumentViewerStatePayload and commit the result."
)
...
class TestTheContractDescribesTheRealPayload:
    def test_a_real_bridge_snapshot_validates(self):
        errors = validate_payload(_snapshot(), DocumentViewerStatePayload)
...
class TestValidatorActuallyRejects:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (lambda p: p.pop("content"), "missing required field"),
            (lambda p: p.__setitem__("content", 12345), "expected string"),
            (lambda p: p.__setitem__("revision", "not a number"), "expected integer"),
    ...
    def test_mutation_is_caught(self, mutate, expected_fragment):
        ...
        errors = validate_payload(payload, DocumentViewerStatePayload)
...
class TestGeneratedArtifactsAreNotStale:
    ...
    def test_schema_matches_regenerating_it_now(self):
        fresh = schema_json_for(DocumentViewerStatePayload, title="DocumentViewerState")
    ...
    def test_typescript_matches_regenerating_it_now(self):
        fresh = typescript_for(DocumentViewerStatePayload, source=_TS_SOURCE_LABEL)
