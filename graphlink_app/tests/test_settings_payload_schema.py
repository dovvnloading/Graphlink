"""The settings island's wire contract: staleness of the generated
artifacts, and that the contract actually describes the REAL payload.
...
_SCHEMA_FILE = _GENERATED / "settings-state.schema.json"
_TS_FILE = _GENERATED / "settings-state.ts"

_TS_SOURCE_LABEL = "graphlink_app/graphlink_settings_payload.py::SettingsStatePayload"

_REGENERATE_HINT = (
    "Regenerate with graphlink_island_codegen.py's schema_json_for()/"
    "typescript_for() against SettingsStatePayload and commit the result."
)
...
def _snapshot(section: str | None = None) -> dict:
    ...
    if section:
        bridge.setActiveSection(section)
    else:
        bridge.ready()
...
class TestValidatorActuallyRejects:
    @pytest.mark.parametrize(
        ...
        [
            (lambda p: p.pop("activeSection"), "missing required field"),
            (lambda p: p.__setitem__("activeSection", 5), "expected string"),
...
class TestGeneratedArtifactsAreNotStale:
    ...
    def test_schema_matches_regenerating_it_now(self):
        fresh = schema_json_for(SettingsStatePayload, title="SettingsState")
    ...
    def test_typescript_matches_regenerating_it_now(self):
        fresh = typescript_for(SettingsStatePayload, source=_TS_SOURCE_LABEL)
