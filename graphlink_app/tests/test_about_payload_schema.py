"""The about-dialog wire contract: staleness of the generated artifacts, and
...
from graphlink_about_payload import AboutStatePayload
...
_SCHEMA_FILE = _GENERATED / "about-state.schema.json"
_TS_FILE = _GENERATED / "about-state.ts"

_TS_SOURCE_LABEL = "graphlink_app/graphlink_about_payload.py::AboutStatePayload"

_REGENERATE_HINT = (
    "Regenerate with graphlink_island_codegen.py's schema_json_for()/"
    "typescript_for() against AboutStatePayload and commit the result."
)
...
class TestTheContractDescribesTheRealPayload:
    def test_a_real_bridge_snapshot_validates(self):
        errors = validate_payload(_snapshot(), AboutStatePayload)
...
class TestValidatorActuallyRejects:
    @pytest.mark.parametrize(
        ...
        [
            (lambda p: p.pop("appName"), "missing required field"),
            (lambda p: p.__setitem__("appVersion", 123), "expected string"),
    ...
    def test_mutation_is_caught(self, mutate, expected_fragment):
        ...
        errors = validate_payload(payload, AboutStatePayload)
...
class TestGeneratedArtifactsAreNotStale:
    ...
    def test_schema_matches_regenerating_it_now(self):
        fresh = schema_json_for(AboutStatePayload, title="AboutState")
    ...
    def test_typescript_matches_regenerating_it_now(self):
        fresh = typescript_for(AboutStatePayload, source=_TS_SOURCE_LABEL)
