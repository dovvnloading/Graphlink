"""The grid-control wire contract: staleness of the generated artifacts, and
...
from graphlink_grid_control_bridge import GridControlBridge
from graphlink_grid_control_payload import GridControlStatePayload
...
_SCHEMA_FILE = _GENERATED / "grid-control-state.schema.json"
_TS_FILE = _GENERATED / "grid-control-state.ts"

_TS_SOURCE_LABEL = "graphlink_app/graphlink_grid_control_payload.py::GridControlStatePayload"

_REGENERATE_HINT = (
    "Regenerate with graphlink_island_codegen.py's schema_json_for()/"
    "typescript_for() against GridControlStatePayload and commit the result."
)
...
class TestTheContractDescribesTheRealPayload:
    def test_a_real_bridge_snapshot_validates(self):
        errors = validate_payload(_snapshot(), GridControlStatePayload)
...
class TestValidatorActuallyRejects:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (lambda p: p.pop("gridSize"), "missing required field"),
            (lambda p: p.pop("colorPresets"), "missing required field"),
            (lambda p: p.__setitem__("sizePresets", "not a list"), "expected array"),
            (lambda p: p["sizePresets"].__setitem__(0, "not an int"), "expected integer"),
    ...
    def test_mutation_is_caught(self, mutate, expected_fragment):
        ...
        errors = validate_payload(payload, GridControlStatePayload)
...
class TestGeneratedArtifactsAreNotStale:
    ...
    def test_schema_matches_regenerating_it_now(self):
        fresh = schema_json_for(GridControlStatePayload, title="GridControlState")
    ...
    def test_typescript_matches_regenerating_it_now(self):
        fresh = typescript_for(GridControlStatePayload, source=_TS_SOURCE_LABEL)
