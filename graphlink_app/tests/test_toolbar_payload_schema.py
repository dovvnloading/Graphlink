"""The toolbar wire contract: staleness of the generated artifacts, and that
the contract actually describes the REAL payload.
...
from graphlink_toolbar_payload import ToolbarStatePayload
...
_SCHEMA_FILE = _GENERATED / "toolbar-state.schema.json"
_TS_FILE = _GENERATED / "toolbar-state.ts"

_TS_SOURCE_LABEL = "graphlink_app/graphlink_toolbar_payload.py::ToolbarStatePayload"

_REGENERATE_HINT = (
    "Regenerate with graphlink_island_codegen.py's schema_json_for()/"
    "typescript_for() against ToolbarStatePayload and commit the result."
)
...
def _snapshot() -> dict:
    bridge = ToolbarBridge(_FakeWindow())
    states: list[str] = []
...
class TestTheContractDescribesTheRealPayload:
    def test_a_real_bridge_snapshot_validates(self):
        errors = validate_payload(_snapshot(), ToolbarStatePayload)

        assert errors == [], errors

    def test_mode_options_is_a_list_of_plain_strings(self):
        options = _snapshot()["modeOptions"]

        assert options == ["Ollama (Local)", "Llama.cpp (Local)", "API Endpoint"]
...
class TestValidatorActuallyRejects:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (lambda p: p.pop("pinsChecked"), "missing required field"),
            (lambda p: p.__setitem__("pinsChecked", "not a bool"), "expected boolean"),
            (lambda p: p.__setitem__("modeOptions", "not a list"), "expected array"),
    ...
    def test_mutation_is_caught(self, mutate, expected_fragment):
        ...
        errors = validate_payload(payload, ToolbarStatePayload)
...
class TestGeneratedArtifactsAreNotStale:
    ...
    def test_schema_matches_regenerating_it_now(self):
        fresh = schema_json_for(ToolbarStatePayload, title="ToolbarState")
    ...
    def test_typescript_matches_regenerating_it_now(self):
        fresh = typescript_for(ToolbarStatePayload, source=_TS_SOURCE_LABEL)
