...
class PluginSpec:
    ...
    # Whether graphlink_window_actions.py's instantiate_seeded_plugin/_seed_plugin_prompt
    # flow can create this plugin from a single parent node plus a starter prompt. False
    # for plugins with a different creation contract (e.g. System Prompt attaches to the
    # graph root rather than branching from a single selected parent).
    seedable: bool
...
class PluginPortal:
    ...
    def execute_plugin(self, plugin_name):
        ...
        # graphlink_window_actions.py restoring state and returning with zero feedback,
    ...
    def create_node(
        ...
        *,
        ...
        # elsewhere in graphlink_scene.py discover nodes via the scene's own per-type
