...
class CustomTitleBar(QWidget):
    def __init__(self, parent=None):
        ...
        icon_label = QLabel()
        icon_pixmap = QPixmap(str(asset_path("graphlink.ico"))).scaled(32, 32, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
...
class _LegacyNotificationBanner(QWidget):
    ...
    def show_message(self, message, duration_ms=5000, msg_type="info"):
        ...
        accent_color = get_semantic_color(f"status_{msg_type}" if f"status_{msg_type}" in {"status_info","status_success","status_error","status_warning"} else "status_info").name()
        if msg_type == "error":
            duration_ms = 0
